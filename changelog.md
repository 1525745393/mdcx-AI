...
## 220260563 (2026-05-31)

### 维护更新
- **版本发布准备**：更新版本号和文档
- **发布规范文档**：添加发布前检查和发布计划的规范文档
- **项目代码规范**：完善项目开发文档和检查流程

---

## 220260562 (2026-05-31)

### 新功能
- **VSMETA 模板语法增强**：大幅提升自定义模板的表达能力
  - 条件渲染语法：`{if:field}内容{/if}` - 仅在字段有值时显示
  - 默认值语法：`{field|默认值}` - 字段为空时使用默认值
  - 扩展占位符：从 11 个增加到 19 个（新增 score, country, director, genre, mosaic, runtime, label, website）
  - 完整的模板验证：语法检查和友好错误提示

### 新增模块
- **VSMETA 模板辅助模块** ([mdcx/utils/vsmeta_template_helper.py](file:///workspace/mdcx/utils/vsmeta_template_helper.py))
  - 预设模板库：标题 7 个、副标题 7 个、简介 5 个
  - 占位符带描述列表：方便用户选择和理解
  - 模块化设计：便于 UI 集成和未来扩展

### 改进
- 模板渲染引擎优化：使用正则表达式处理复杂语法
- 保持 100% 向后兼容：现有模板无需修改
- 代码重构：分离预设和验证逻辑，提高可维护性

---

## 220260561 (2026-05-31)

### 新功能
- **VSMETA 自定义模板功能**：允许用户自定义标题、副标题、简介的组合格式
  - 支持自定义分隔符
  - 支持模板语法（如 `{number} - {title} ({originaltitle})`）
  - 支持 11 种占位符：number, title, originaltitle, publisher, studio, series, actors, outline, originalplot, year, release
  - UI 界面添加自定义模板输入控件
  - 配置保存和加载功能完整
...
### 改进
- VSMETA 标题格式扩展：新增"仅番号"、"番号 + 日文原始标题"、"中文标题 + 日文标题"、"日文标题 + 中文标题"等选项
