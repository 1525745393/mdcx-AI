...
### 改进
- **不可恢复的错误立即停止重试**：对于 SSL/TLS 库配置错误等不可恢复的错误，不再无意义地重试
  - 添加 `is_non_retryable_error()` 函数检测不可重试的错误
  - 遇到 "OPENSSL_internal:invalid library" 等错误时立即停止重试
  - 避免程序在配置错误的 SSL 库上一直重复请求
  - 提高错误处理效率，减少无意义的网络请求
